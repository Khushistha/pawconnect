...
reportsRouter.patch('/reports/:id/status', requireAuth, async (req, res, next) => {
  try {
    ...
    // If this report is linked to a dog, keep the dog status in sync for key transitions.
    // Completed rescue reports should surface in the adoption listing, so mark linked dog as adoptable.
    // Also: backfill dogs.created_by (NGO handler) if missing so adoption notifications route correctly.
    if (row.dog_id && status === 'completed') {
      const reviewerId = req.user?.sub || req.user?.id || null;
      await pool.query(
        `UPDATE dogs 
         SET status = 'adoptable',
             created_by = COALESCE(created_by, ?)
         WHERE id = ? AND status <> 'adopted'`,
        [reviewerId, row.dog_id]
      );
    }
...
reportsRouter.post('/reports/:id/create-dog', requireAuth, async (req, res, next) => {
  try {
    const userRole = req.user?.role;
    if (userRole !== 'ngo_admin' && userRole !== 'superadmin') {
      throw new HttpError(403, 'Only NGO admins and superadmins can create dogs from reports');
    }
    ...
    if (report.dog_id) {
      throw new HttpError(400, 'A dog has already been created from this report');
    }
    ...
    const reportPhotos = reportPhotoRows.map(p => p.url);

    // Create dog from report data
    const dogId = uuidv4();
    ...
    await pool.query(
      ...
      [
        ...
        dogStatus,
        report.description,
        `Rescued from report by ${report.reported_by}`,
        report.location_lat,
        report.location_lng,
        report.location_address,
        report.location_district ?? null,
        0,
        0,
    ...
    // Copy photos from report to dog
    if (reportPhotos.length > 0) {
      const photoValues = reportPhotos.map((url, idx) => [dogId, url, idx]);
      await pool.query(
    ...
    // Link the report to the dog
    await pool.query(
      `UPDATE rescue_reports SET dog_id = ? WHERE id = ?`,
      [dogId, reportId]
    );
    ...
    const dogRow = dogRows?.[0];
    if (!dogRow) throw new HttpError(500, 'Failed to create dog');

    const [dogPhotoRows] = await pool.query(
    ...
    // Map dog row (same logic as in dogs.js)
    const mapDogRow = (row, photoUrls = [], reportInfo = null) => {
      return {
        id: row.id,
        name: row.name,
    ...
    res.status(201).json({ 
      item: mapDogRow(dogRow, dogPhotoRows.map((p) => p.url), reportInfo),
